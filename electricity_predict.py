...
class ELECTRIcity:
    ...
    def predict(self, input_seq):
        ...
        for i in range(len(self.appliances)):
            ...
            cutoff = torch.tensor(self.args[i].cutoff[self.appliances[i]]).to(self.device)
            threshold = torch.tensor(self.args[i].threshold[self.appliances[i]]).to(self.device)

            _, _, y, s = self.get_model_outputs(self.models[self.appliances[i]], agg, cutoff, threshold)
            y = y * s
